class Imagekit::PreloadedFile
  PRELOADED_IMAGEKIT_PATH = /^([^\/]+)\/([^\/]+)\/v(\d+)\/([^#]+)#([^\/]+)$/

  attr_reader :filename, :version, :public_id, :signature, :resource_type, :type
  def initialize(file_info)
    @resource_type, @type, @version, @filename, @signature = file_info.scan(PRELOADED_IMAGEKIT_PATH).first    
    @public_id, @format = Imagekit::PreloadedFile.split_format(@filename)      
  end
  
  def valid?
    public_id = @resource_type == "raw" ? self.filename : self.public_id
    expected_signature = Imagekit.generate_signature({apiKey: Imagekit.configuration.public_key, filename: filename, timestamp: Time.now})
