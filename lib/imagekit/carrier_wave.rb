require 'imagekit/carrier_wave/process'
require 'imagekit/carrier_wave/error'
require 'imagekit/carrier_wave/remote'
require 'imagekit/carrier_wave/preloaded'
require 'imagekit/carrier_wave/storage'

module Imagekit::CarrierWave

  def self.included(base)
    base.storage Imagekit::CarrierWave::Storage
  ...
  def retrieve_from_store!(identifier)
    ...
    if identifier.blank? || mongoid_blank
      ...
      @file = ImagekitFile.new(identifier, self)
  ...
  def rename(to_public_id = nil, overwrite=false)
    ...
    Imagekit::Uploader.rename(from_public_id, to_public_id, :type=>self.storage_type, :resource_type=>self.resource_type, :overwrite=>overwrite)
    storage.store_imagekit_identifier(@stored_version, [@public_id, self.format].join("."))
  end
  ...
  # Should removed files be removed from Imagekit as well. Can be overridden.
  def delete_remote?
    true
  end

  # Let Imagekit download remote URLs directly
  def imagekit_should_handle_remote?
    true
  end
  ...
  class ImagekitFile
    ...
    def initialize(identifier, uploader)
      ...
      @resource_type ||= Imagekit::Utils.resource_type_for_format(@filename)
    ...
    def delete
      ...
      # Imagekit::Uploader.destroy(public_id, :type=>self.storage_type, :resource_type=>self.resource_type) if @uploader.delete_remote?
    end

    def exists?
      public_id = @resource_type == "raw" ? self.filename : self.public_id
      # Imagekit::Uploader.exists?(public_id, :version=>self.version, :type=>self.storage_type, :resource_type=>self.resource_type)
    end

    def read(options={})
      parameters={:type=>self.storage_type, :resource_type=>self.resource_type}.merge(options)
      # Imagekit::Downloader.download(self.identifier, parameters)
    end
  ...
  def self.split_format(identifier)
    return Imagekit::PreloadedFile.split_format(identifier)
  end
  ...
  def resource_type
    @file.respond_to?(:resource_type) ? @file.resource_type : Imagekit::Utils.resource_type_for_format(requested_format || original_filename || default_format)
  end
